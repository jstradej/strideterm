...
function savePrefixPreference() {
  try { localStorage.setItem(PREFIX_STORAGE_KEY, branchPrefix.value); } catch { /* ignore */ }
}
...
const validationError = computed(() => {
  ...
  if (BRANCH_INVALID_PATTERNS.test(name)) return "Invalid branch name pattern (no .., //, leading/trailing dots or slashes).";
...
async function loadBranches() {
  ...
  try {
    ...
    const preferred = branches.value.find((b) => b === "develop")
