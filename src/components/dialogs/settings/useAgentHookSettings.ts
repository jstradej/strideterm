...
export function useAgentHookSettings(api: any) {
  ...
  const providers = [
    createHookProvider(api, {
      ...
      configureTitle:
        "Install strIDEterm hook entries into ~/.claude/settings.json so Claude Code fires Notification/Stop/SubagentStop/UserPromptSubmit events (plus a Task/Agent-scoped PreToolUse so subagents show as running, and PermissionRequest so alerts can name the tool being approved) to the local listener. Merges with existing user hooks.",
