  ...
  <div class="workspace-pane__body workspace-pane__body--git">
    ...
    <div v-if="!connections.length" class="terminal-empty" style="align-content:start;padding-top:32px;">
      ...
      <div class="docker-card__actions" style="margin-top:12px;">
        <button type="button" class="button" @click="appStore.openGitHubConnectionDialog('')">Add GitHub connection</button>
      </div>
    </div>
    <div v-else class="azure-inbox">
      ...
      <div v-if="openError" style="padding:8px 12px;font-size:12px;background:rgba(255,80,80,0.08);border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;">
        <span style="color:var(--danger,#e53935);">{{ openError }}</span>
        <button type="button" class="button button--ghost" style="font-size:10px;padding:1px 8px;margin-left:auto;" @click="openError = ''">Dismiss</button>
      </div>
        ...
        <section v-for="tab in inboxTabs" :key="tab.id" :class="['azure-section', activeTab === tab.id && 'azure-section--active']">
          ...
          <template v-if="tab.id === 'connections' && activeTab === 'connections'">
            ...
            <div class="docker-list azure-connection-list" style="gap:8px;">
              <article v-for="conn in connections" :key="conn.id" class="docker-card" :style="`border-left:3px solid ${conn.status === 'ok' ? '#238636' : 'var(--muted)'};`">
                ...
                <div style="font-size:12px;color:var(--muted);padding:4px 0;">
                  {{ conn.ownerFilters?.length ? conn.ownerFilters.join(', ') : 'all owners' }}{{ conn.repositoryFilters?.length ? ` · ${conn.repositoryFilters.join(', ')}` : '' }} · poll {{ conn.pollSeconds || 120 }}s
                </div>
                <div v-if="conn.lastError" style="font-size:11px;color:var(--danger);padding:2px 0;">{{ conn.lastError }}</div>
                ...
                <div class="docker-card__actions">
                  <button type="button" class="button button--ghost" @click="appStore.openGitHubConnectionDialog(conn.id)">Edit</button>
          ...
          <template v-else-if="activeTab === tab.id">
            <div v-if="repoNames.length > 1 && tabItems(activeTab).length" style="display:flex;gap:4px;padding:0 12px 8px;flex-wrap:wrap;">
              <button type="button" :class="['button', 'button--ghost']" :style="!repoFilter ? 'font-size:11px;padding:2px 8px;background:var(--accent);color:var(--bg);' : 'font-size:11px;padding:2px 8px;'" @click="repoFilter = ''">All repos</button>
              <button v-for="repo in repoNames" :key="repo" type="button" :class="['button', 'button--ghost']" :style="repoFilter === repo ? 'font-size:11px;padding:2px 8px;background:var(--accent);color:var(--bg);' : 'font-size:11px;padding:2px 8px;'" @click="repoFilter = repoFilter === repo ? '' : repo">{{ repo }}</button>
            </div>
...
async function handleRefresh() {
  ...
  try { await appStore.refreshGitHub(); }
  finally { busyAction.value = ""; }
}

async function handleDeleteConnection(connId) {
  busyAction.value = `delete-${connId}`;
  try { await appStore.deleteGitHubConnection(connId); }
  finally { busyAction.value = ""; }
}
