      ...
      <header class="docker-manager__header">
        ...
        <div style="display: flex; gap: 6px; flex-wrap: wrap">
          <button
            ...
            @click="appStore.openLazydocker(workspaceId)"
          >
            Open Lazydocker
          </button>
          <button
            ...
            title="Install lazydocker: winget install JesseDuffield.lazygit or brew install lazydocker"
          >
            Install Lazydocker
          </button>
        </div>
          ...
          <li v-for="container in containers" :key="container.ID">
            <article :class="['docker-card', isRunning(container) && 'docker-card--running']">
              <div class="docker-card__head">
                ...
                <span :class="['docker-state', `docker-state--${isRunning(container) ? 'running' : 'stopped'}`]">{{
                  container.State || (isRunning(container) ? "running" : "stopped")
                }}</span>
              </div>
              ...
              <div class="docker-card__actions" aria-label="Container actions">
                <button
                  type="button"
                  :class="['button', 'button--ghost', busyAction === `shell-${container.ID}` && 'button--busy']"
                  :disabled="!isRunning(container) || !!busyAction"
                  @click="handleDockerShell(container.ID)"
                >
                  Shell
                </button>
                <button
                  type="button"
                  :class="['button', 'button--ghost', busyAction === `logs-${container.ID}` && 'button--busy']"
                  :disabled="!!busyAction"
                  @click="handleDockerLogs(container.ID)"
                >
                  Logs
                </button>
                <button
                  type="button"
                  :class="['button', 'button--ghost', busyAction === `docker-start-${container.ID}` && 'button--busy']"
                  :disabled="isRunning(container) || !!busyAction"
                  @click="handleDockerAction('docker-start', container.ID)"
                >
                  {{ busyAction === `docker-start-${container.ID}` ? "Starting…" : "Start" }}
                </button>
                <button
                  type="button"
                  :class="['button', 'button--ghost', busyAction === `docker-stop-${container.ID}` && 'button--busy']"
                  :disabled="!isRunning(container) || !!busyAction"
                  @click="handleDockerAction('docker-stop', container.ID)"
                >
                  {{ busyAction === `docker-stop-${container.ID}` ? "Stopping…" : "Stop" }}
                </button>
                <button
                  type="button"
                  :class="[
                    'button',
                    'button--ghost',
                    busyAction === `docker-restart-${container.ID}` && 'button--busy',
                  ]"
                  :disabled="!isRunning(container) || !!busyAction"
                  @click="handleDockerAction('docker-restart', container.ID)"
                >
                  {{ busyAction === `docker-restart-${container.ID}` ? "Restarting…" : "Restart" }}
                </button>
                <button
                  type="button"
                  :class="[
                    ...
                    'button--ghost',
                    'danger',
                    busyAction === `docker-remove-${container.ID}` && 'button--busy',
                  ]"
                  :disabled="!!busyAction"
                  @click="handleDockerAction('docker-remove', container.ID)"
                >
                  {{ busyAction === `docker-remove-${container.ID}` ? "Removing…" : "Remove" }}
                </button>
              </div>
            </article>
          </li>
