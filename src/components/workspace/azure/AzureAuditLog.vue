  ...
  <div class="azure-audit-log">
    ...
    <div class="azure-audit-log__filters">
      ...
      <input
        v-model="searchText"
        type="text"
        class="azure-audit-log__search"
        placeholder="Search\u2026"
        @input="onSearchInput"
      />
      <button
        type="button"
        :class="['button', 'button--ghost', loading && 'button--busy']"
        :disabled="loading"
        @click="loadData"
      >
    ...
    <div v-if="stats.total" class="azure-audit-log__stats">
      <span
        ><strong>{{ stats.total }}</strong> calls</span
      >
          ...
          <tr>
            <th v-for="(col, ci) in columns" :key="col.key" @click="toggleSort(col.key)">
              <div class="azure-audit-log__th-inner">
                <span>{{ col.label }}</span>
                <span v-if="sortKey === col.key" class="azure-audit-log__sort-arrow">{{
                  sortDir === "asc" ? "\u25B2" : "\u25BC"
                }}</span>
              </div>
              <div
                v-if="ci < columns.length - 1"
                class="azure-audit-log__resize-handle"
                @mousedown.stop.prevent="startResize($event, ci)"
              ></div>
            </th>
          </tr>
        </thead>
          ...
          <template v-for="entry in sortedEntries" :key="entry.id">
            <tr
              :class="[
                ...
                !entry.success && 'azure-audit-log__row--error',
                expandedId === entry.id && 'azure-audit-log__row--expanded',
              ]"
            ...
            >
              ...
              <td>
                <span :class="['azure-audit-log__cat', `azure-audit-log__cat--${entry.category}`]">{{
                  entry.category
                }}</span>
              </td>
              <td class="azure-audit-log__cell-project">
                {{
                  isGitHub ? (entry.owner ? `${entry.owner}/${entry.repository}` : "\u2014") : entry.project || "\u2014"
                }}
              </td>
              <td>
                <span v-if="entry.success" class="azure-audit-log__status-ok">{{ entry.statusCode || "OK" }}</span>
                <span v-else class="azure-audit-log__status-err" :title="entry.errorMessage || ''">{{
                  entry.statusCode || "ERR"
                }}</span>
              </td>
              <td class="azure-audit-log__cell-dur">
                {{ entry.durationMs != null ? entry.durationMs + "ms" : "\u2014" }}
              </td>
              <td>
                <span v-if="entry.userInitiated" class="workspace-chip" style="font-size: 9px; padding: 0 4px"
                  >user</span
                >
              ...
              <td colspan="7">
                <div class="azure-audit-log__detail">
                  <div class="azure-audit-log__detail-grid">
                    ...
                    <span
                      ><strong>{{ entry.method }}</strong> {{ entry.statusCode || "" }}</span
                    >
...
.azure-audit-log__stat-ok {
  color: var(--accent);
}
.azure-audit-log__stat-err {
  color: var(--error, #e55);
}
