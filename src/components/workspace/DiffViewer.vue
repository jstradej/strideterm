<template>
  <pre class="git-output git-output--preview"><template v-for="(line, index) in visibleLines" :key="index"><template v-if="index > 0">
...
<span class="diff-meta">… {{ lines.length - MAX_LINES }} more lines ({{ lines.length }} total)</span></template></pre>
  <button v-if="isTruncated && !showAll" type="button" class="button button--ghost" style="margin:6px 0;font-size:12px;" @click="showAll = true">Show all {{ lines.length }} lines</button>
</template>
...
watch(() => props.diff, () => { showAll.value = false; });
