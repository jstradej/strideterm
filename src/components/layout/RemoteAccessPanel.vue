  ...
  <template v-if="!store.remoteAccessExpanded">
    <div v-if="remoteConfig.enabled && hasAnyUrl" class="remote-card remote-card--compact">
      ...
      <div class="remote-compact__info">
        ...
        <button type="button" class="button button--ghost remote-summary__configure" @click="store.toggleRemotePanel()">Configure</button>
      </div>
    </div>
  ...
  <div v-if="store.remoteAccessExpanded" :class="['remote-card', 'remote-access--expanded', !remoteConfig.enabled && 'remote-card--disabled']">
      ...
      <div class="remote-access__hero-copy">
        <label class="remote-access__field">
          ...
          <span class="remote-field">
            ...
            <button type="button" class="button button--ghost remote-field__copy" :disabled="!modeShareUrl" @click="copyModeUrl">Copy</button>
          </span>
        </label>
      </div>
    </div>
    ...
    <div class="remote-access__body">
      ...
      <div :class="['remote-mode-panel', store.remoteAccessMode === 'lan' && 'remote-mode-panel--active']">
        <p class="remote-access__headline" :title="`If you can't connect from another device, check that Windows Firewall allows inbound TCP on port ${lanPort}.`">
        ...
        <label class="remote-access__field">
          ...
          <span class="remote-field">
            ...
            <button type="button" class="button button--ghost remote-field__copy" :disabled="!lanShareUrl" @click="store.copyText(lanShareUrl)">Copy</button>
          </span>
        </label>
        ...
        <div class="remote-access__stats">
          ...
          <span class="workspace-chip"><strong>{{ lanPort }}</strong> port</span>
      ...
      <div :class="['remote-mode-panel', store.remoteAccessMode === 'cloudflare' && 'remote-mode-panel--active']">
        ...
        <label class="remote-access__field">
          ...
          <span class="remote-field">
            ...
            <button type="button" class="button button--ghost remote-field__copy" :disabled="!tunnelShareUrl" @click="store.copyText(tunnelShareUrl)">Copy</button>
          </span>
        </label>
        <template v-if="!isRemote">
          <label class="remote-access__field">
            ...
            <span class="remote-field">
              <input
                v-model="cloudflaredPathInput"
                placeholder="Leave empty for PATH, or set full path"
              />
              <button type="button" class="button button--ghost remote-field__copy" @click="browseCloudflared">Browse</button>
            </span>
          </label>
        </template>
        <div class="remote-access__stats">
          <span :class="['workspace-chip', tunnelStatusLabel === 'connected' ? 'workspace-chip--ok' : 'workspace-chip--default']">
        ...
        <div v-if="!isRemote" class="remote-mode-panel__actions">
          <button type="button" class="button" :disabled="!(remoteConfig.enabled && tunnel.available)" @click="store.createCloudflareTunnel()">
            {{ tunnel.publicUrl ? 'Recreate tunnel' : 'Create tunnel' }}
          </button>
          <button type="button" class="button button--ghost" :disabled="!remoteConfig.enabled" @click="store.refreshTunnel()">Refresh</button>
          <button type="button" class="button button--ghost" :disabled="!tunnel.publicUrl" @click="store.stopCloudflareTunnel()">Stop</button>
        </div>
      </div>
      ...
      <div :class="['remote-mode-panel', store.remoteAccessMode === 'vps' && 'remote-mode-panel--active']">
        ...
        <label class="remote-access__field">
          ...
          <span class="remote-field">
            ...
            <button type="button" class="button button--ghost remote-field__copy" :disabled="!customShareUrl || !normalizedCustomUrl" @click="store.copyText(customShareUrl)">Copy</button>
          </span>
        </label>
        <div v-if="!isRemote" class="remote-mode-panel__actions">
          <button type="button" class="button" @click="saveCustomUrl">Save URL</button>
          <button type="button" class="button button--ghost" :disabled="!remoteConfig.customPublicUrl" @click="store.clearCustomPublicUrl()">Clear</button>
        </div>
      </div>
    </div>
    ...
    <div class="remote-access__footer">
      ...
      <div v-if="!isRemote" class="remote-access__footer-actions">
        <button type="button" class="button button--ghost" :disabled="!remoteConfig.enabled" @click="store.regenerateRemoteToken()">Regenerate token</button>
      </div>
    </div>
  </div>
  </Teleport>
...
const {
  ...
  lanShareUrl, tunnelShareUrl, customShareUrl, customPublicUrl, normalizedCustomUrl,
  modeShareUrl, activeShareUrl,
