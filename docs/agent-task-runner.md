...
4. Once those validate, the baseline evaluation starts on its own — the capture is slow enough that stopping for a second confirmation is what surprises people. Untick **Start the loop as soon as the brief is ready** in the create dialog to stop at the **Brief ready** screen instead and review or edit the captured brief before any round is spent
...
| **Brief ready**              | Captured brief is ready for your review before the loop starts. Skipped when the task was created with auto-start (the state is still entered, so a loop whose auto-start is refused — e.g. the Primary went away mid-capture — lands in the normal manual gate rather than being stranded)                                                                                                                                                                                                                                  |
...
**Verdict contract.** `verdict.json` is validated against a schema far richer than the standard task's `{verdict, reason}` — nested `verificationReview`, a role-specific `roleAnalysis`, findings/advisories/questions with their own id patterns. The evaluation prompt therefore carries that schema in full, generated from the zod definition itself and scoped to the one role being asked, so it cannot drift from what the runner enforces. Cross-field rules (`complete` ⇒ no blockers, `continue` ⇒ at least one, Planner ⇒ never `needs-input`) have no JSON Schema form and stay spelled out in the prose beside it. A malformed verdict gets the same schema back with the validation errors, and the Companion has a small budget of rewrites before the task pauses — only a verdict it actually changed spends one, so the several idle signals that arrive per turn can't exhaust it. This exists because the prompt used to describe the verdict as `{"verificationReview": {...}, ...}` and nothing more: every nested field was a guess, one rejection was the whole budget, and a Companion once went reading strideterm's own sources to find the schema instead of reviewing the task it was given.

**Completion floor.** From the first round-review on, every role except Planner can only reach `complete` against a `VERIFICATION.md` for the current round that the runner itself read as fresh. This is checked twice: the verdict schema rejects a `complete` whose `recordStatus` isn't `fresh`, and the runner then compares that claim against the record it actually handed to that evaluation — so a review can't sign off by simply claiming one exists. The **baseline** is exempt from the schema half only: it runs before any verification round exists, so there is never a record for it to be fresh against, and enforcing it there made a clean first review impossible to express — it had to come back as a `continue` carrying an invented blocker whose whole content was "now record the evidence", consuming a round for something that is not a defect. The runtime half still applies, so a clean baseline is withheld rather than signed off; it just takes the path below instead. When the claim doesn't hold, the review is kept but the sign-off is withheld: the Primary is asked to record the evidence for the same round, no round is consumed, and the next round-review can complete. A round that genuinely needs no command still satisfies the floor — a fresh record whose "Checks not run" section says why.
